...
data "archive_file" "test-function-source" {
  type        = "zip"
  source_dir  = "${path.module}/lambdas/test"
  output_path = "${path.module}/lambdas/test-tf-handler-${random_uuid.archive.result}.zip"
}

# Upload Lamda function to S3
resource "aws_s3_bucket_object" "test-function-storage-upload" {
  bucket = var.dev_bucket_id
  key    = "lambdas/test-tf-handler.zip"
  source = data.archive_file.test-function-source.output_path
  etag   = filemd5(data.archive_file.test-function-source.output_path)
}

# Lamda Initialization
resource "aws_lambda_function" "test-function-handler" {
  function_name     = "test-handler"
  description       = "${var.aws_profile}: test"
  s3_bucket         = var.dev_bucket_id
  s3_key            = aws_s3_bucket_object.test-function-storage-upload.key
  ...
  role              = var.default_lambda_role_arn
  publish           = true
  source_code_hash  = filebase64sha256(data.archive_file.test-function-source.output_path)
}

# Give permission to the API gateway to access this lambda
resource "aws_lambda_permission" "test-api-gateway" {
  statement_id  = "AllowAPIGatewayInvoke"

  # Name of lambda from above
  function_name = aws_lambda_function.test-function-handler.arn
  action        = "lambda:InvokeFunction"
  principal     = "apigateway.amazonaws.com"
